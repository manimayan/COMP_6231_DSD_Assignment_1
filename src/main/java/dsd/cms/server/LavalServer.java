...
/**
 * The Class LavalServer.
 */
public class LavalServer extends UnicastRemoteObject implements CenterServer {
	...
	/**
	 * Instantiates a new laval server.
	 *
	 * @throws RemoteException the remote exception
	 */
	protected LavalServer() throws RemoteException {
		super();
		this.mainServer = new MainServer();
		this.teacherStudentRecords = this.mainServer.loadData("src/main/resources/LavalData.txt");
	}

	/** The logger. */
	private static Logger logger = Logger.getLogger("Laval");
	 ...
	 *                       performed
	 * @param lavalServerArg the laval server arg
	 * @throws RemoteException       the remote exception
	 * @throws AlreadyBoundException the already bound exception
	 */
	public void serverConnection(int port, LavalServer lavalServerArg) throws RemoteException, AlreadyBoundException {

		LavalServer lavalServer = lavalServerArg;
		Registry registry2 = LocateRegistry.createRegistry(9992);
		registry2.bind("Laval", lavalServer);

		logger.info("Laval Server Started");
		System.out.println("\nLaval server is loaded with initial data. Map size : " + teacherStudentRecords.size());

		logWriter.serverInfo(LogWriter.LAVAL, LogWriter.SYSTEM, LogWriter.PHASE_STARTUP, "Laval server started");
		logWriter.serverInfo(LogWriter.LAVAL, LogWriter.SYSTEM, LogWriter.PHASE_STARTUP, "Initial data loaded.");
		...
		while (true) {
			try (DatagramSocket ds = new DatagramSocket(port)) {
				...
				if (serviceName.trim().equalsIgnoreCase("getRecordCounts")) {
					outputStr = "LVL : " + this.mainServer.getRecordCount(this.teacherStudentRecords);
				}
	...
	public synchronized String createTRecord(String firstName, String lastName, String address, String phone,
		...
		logWriter.serverInfo(LogWriter.LAVAL, clientId, LogWriter.PHASE_REQUEST,
		...
		String resultMsg = this.mainServer.persistTeacherRecordInMap(teacherRecord, this.teacherStudentRecords)
				? "Teacher record " + teacherRecord.getRecordID() + " created in Laval location with name : "
						+ firstName + " " + lastName
				: "Error in creating Teacher Record.";
		System.out.println(resultMsg);
		logWriter.serverInfo(LogWriter.LAVAL, clientId, LogWriter.PHASE_RESPONSE, resultMsg);
		return resultMsg;
	}
	...
	public synchronized String createSRecord(String firstName, String lastName, String courseRegistered, String status,
		...
		String resultMsg = this.mainServer.persistStudentRecordInMap(studentRecord, this.teacherStudentRecords)
				? "Student record " + studentRecord.getRecordID() + " created in Laval location with name : "
						+ firstName + " " + lastName
	...
	public synchronized String getRecordCounts(String clientId) {
		...
		String server2 = "LVL : " + this.mainServer.getRecordCount(teacherStudentRecords);
